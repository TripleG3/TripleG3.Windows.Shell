...
/// <summary>
/// Provides a version-tolerant wrapper over the native Windows <c>Winscard.dll</c> module for smart card resource manager APIs.
/// </summary>
/// <remarks>
/// The set of exported <c>Winscard.dll</c> functions can differ between Windows versions, installed smart card components, and reader support.
...
/// <c>TryGetFunction&lt;TDelegate&gt;</c>. Smart card APIs interact with reader hardware and card state; use this type only
/// when code needs direct access to Windows smart card context, reader, card, and transaction exports.
/// </remarks>
public static class Winscard
{
    /// <summary>The canonical Windows module name for Winscard.</summary>
    public const string LibraryName = "Winscard.dll";

    private static readonly NativeModule s_module = new(LibraryName, typeof(Winscard));
    private static readonly Lazy<IReadOnlyList<Export>> s_exports = new(GetExports, LazyThreadSafetyMode.ExecutionAndPublication);

    /// <summary>Gets the loaded native module handle for <c>Winscard.dll</c>.</summary>
    /// <exception cref="DllNotFoundException">Thrown when Windows cannot load <c>Winscard.dll</c>.</exception>
    public static nint ModuleHandle => s_module.ModuleHandle;

    /// <summary>Gets the full path to the loaded <c>Winscard.dll</c> module.</summary>
    public static string ModulePath => s_module.ModulePath;

    /// <summary>Gets metadata for every exported <c>Winscard.dll</c> function available on the current machine.</summary>
    public static IReadOnlyList<Export> Exports => s_exports.Value;

    /// <summary>Gets every named <c>Winscard.dll</c> export available on the current machine.</summary>
    public static IReadOnlyList<string> ExportNames => s_module.ExportNames;

    /// <summary>Attempts to resolve a named <c>Winscard.dll</c> export to its native function pointer.</summary>
    ...
    /// <summary>Resolves a named <c>Winscard.dll</c> export to its native function pointer.</summary>
    /// <param name="name">The exact exported function name, including any ANSI/Unicode suffix such as <c>A</c> or <c>W</c>.</param>
    /// <returns>The native function pointer for the requested export.</returns>
    /// <exception cref="EntryPointNotFoundException">Thrown when the export does not exist in the loaded <c>Winscard.dll</c>.</exception>
    public static nint GetExport(string name)
    {
        return s_module.GetExport(name);
    }

    /// <summary>Attempts to resolve a <c>Winscard.dll</c> export by ordinal to its native function pointer.</summary>
    ...
    /// <summary>Resolves a <c>Winscard.dll</c> export by ordinal to its native function pointer.</summary>
    /// <param name="ordinal">The 16-bit export ordinal.</param>
    /// <returns>The native function pointer for the requested export.</returns>
    /// <exception cref="EntryPointNotFoundException">Thrown when the ordinal does not exist in the loaded <c>Winscard.dll</c>.</exception>
    public static nint GetExport(int ordinal)
    {
        return s_module.GetExport(ordinal);
    }

    /// <summary>Resolves a named <c>Winscard.dll</c> export and converts it to a managed delegate.</summary>
    ...
    /// <see cref="CallingConvention.Winapi" /> and the exact context, card handle, protocol, and buffer types from the Windows SDK.
    /// For example, bind <c>SCardEstablishContext</c>, <c>SCardListReadersW</c>, or <c>SCardReleaseContext</c> only to a matching delegate.
    /// </remarks>
    ...
    /// <summary>Attempts to resolve a named <c>Winscard.dll</c> export and convert it to a managed delegate.</summary>
    /// <typeparam name="TDelegate">The managed delegate type that exactly matches the native function signature.</typeparam>
    /// <param name="name">The exact exported function name, including any ANSI/Unicode suffix such as <c>A</c> or <c>W</c>.</param>
    ...
    /// <summary>Resolves a <c>Winscard.dll</c> export by ordinal and converts it to a managed delegate.</summary>
    /// <typeparam name="TDelegate">The managed delegate type that exactly matches the native function signature.</typeparam>
    /// <param name="ordinal">The 16-bit export ordinal.</param>
    /// <returns>A delegate bound to the native function pointer.</returns>
    ...
    /// <summary>Attempts to resolve a <c>Winscard.dll</c> export by ordinal and convert it to a managed delegate.</summary>
    /// <typeparam name="TDelegate">The managed delegate type that exactly matches the native function signature.</typeparam>
    /// <param name="ordinal">The 16-bit export ordinal.</param>
    ...
    /// <summary>Describes one native export from the loaded <c>Winscard.dll</c> module.</summary>
    ...
    /// <param name="ForwardedTo">The forwarded export target, or <see langword="null" /> when the export is implemented in <c>Winscard.dll</c>.</param>
