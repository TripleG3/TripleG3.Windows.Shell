...
/// <summary>
/// Provides a version-tolerant wrapper over the native Windows <c>Advapi32.dll</c> module for registry, service control, security descriptor, LSA, and event logging APIs.
/// </summary>
/// <remarks>
/// The set of exported <c>Advapi32.dll</c> functions can differ between Windows versions and installed Windows features.
...
/// <c>TryGetFunction&lt;TDelegate&gt;</c>. Prefer built-in .NET registry, service, and security APIs for ordinary
/// application code; use this type when code needs direct access to Windows advanced API exports.
/// </remarks>
public static class Advapi32
{
    /// <summary>The canonical Windows module name for Advapi32.</summary>
    public const string LibraryName = "Advapi32.dll";

    private static readonly NativeModule s_module = new(LibraryName, typeof(Advapi32));
    private static readonly Lazy<IReadOnlyList<Export>> s_exports = new(GetExports, LazyThreadSafetyMode.ExecutionAndPublication);

    /// <summary>Gets the loaded native module handle for <c>Advapi32.dll</c>.</summary>
    /// <exception cref="DllNotFoundException">Thrown when Windows cannot load <c>Advapi32.dll</c>.</exception>
    public static nint ModuleHandle => s_module.ModuleHandle;

    /// <summary>Gets the full path to the loaded <c>Advapi32.dll</c> module.</summary>
    public static string ModulePath => s_module.ModulePath;

    /// <summary>Gets metadata for every exported <c>Advapi32.dll</c> function available on the current machine.</summary>
    public static IReadOnlyList<Export> Exports => s_exports.Value;

    /// <summary>Gets every named <c>Advapi32.dll</c> export available on the current machine.</summary>
    public static IReadOnlyList<string> ExportNames => s_module.ExportNames;

    /// <summary>Attempts to resolve a named <c>Advapi32.dll</c> export to its native function pointer.</summary>
    ...
    /// <summary>Resolves a named <c>Advapi32.dll</c> export to its native function pointer.</summary>
    /// <param name="name">The exact exported function name, including any ANSI/Unicode suffix such as <c>A</c> or <c>W</c>.</param>
    /// <returns>The native function pointer for the requested export.</returns>
    /// <exception cref="EntryPointNotFoundException">Thrown when the export does not exist in the loaded <c>Advapi32.dll</c>.</exception>
    public static nint GetExport(string name)
    {
        return s_module.GetExport(name);
    }

    /// <summary>Attempts to resolve a <c>Advapi32.dll</c> export by ordinal to its native function pointer.</summary>
    ...
    /// <summary>Resolves a <c>Advapi32.dll</c> export by ordinal to its native function pointer.</summary>
    /// <param name="ordinal">The 16-bit export ordinal.</param>
    /// <returns>The native function pointer for the requested export.</returns>
    /// <exception cref="EntryPointNotFoundException">Thrown when the ordinal does not exist in the loaded <c>Advapi32.dll</c>.</exception>
    public static nint GetExport(int ordinal)
    {
        return s_module.GetExport(ordinal);
    }

    /// <summary>Resolves a named <c>Advapi32.dll</c> export and converts it to a managed delegate.</summary>
    ...
    /// For example, bind <c>RegOpenKeyExW</c>, <c>OpenSCManagerW</c>, or <c>RegisterEventSourceW</c> only to a delegate that matches the Windows SDK signature exactly.
    /// </remarks>
    ...
    /// <summary>Attempts to resolve a named <c>Advapi32.dll</c> export and convert it to a managed delegate.</summary>
    /// <typeparam name="TDelegate">The managed delegate type that exactly matches the native function signature.</typeparam>
    /// <param name="name">The exact exported function name, including any ANSI/Unicode suffix such as <c>A</c> or <c>W</c>.</param>
    ...
    /// <summary>Resolves a <c>Advapi32.dll</c> export by ordinal and converts it to a managed delegate.</summary>
    /// <typeparam name="TDelegate">The managed delegate type that exactly matches the native function signature.</typeparam>
    /// <param name="ordinal">The 16-bit export ordinal.</param>
    /// <returns>A delegate bound to the native function pointer.</returns>
    ...
    /// <summary>Attempts to resolve a <c>Advapi32.dll</c> export by ordinal and convert it to a managed delegate.</summary>
    /// <typeparam name="TDelegate">The managed delegate type that exactly matches the native function signature.</typeparam>
    /// <param name="ordinal">The 16-bit export ordinal.</param>
    ...
    /// <summary>Describes one native export from the loaded <c>Advapi32.dll</c> module.</summary>
    ...
    /// <param name="ForwardedTo">The forwarded export target, or <see langword="null" /> when the export is implemented in <c>Advapi32.dll</c>.</param>
