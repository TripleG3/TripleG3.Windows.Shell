...
/// <summary>
/// Provides a version-tolerant wrapper over the native Windows <c>Iphlpapi.dll</c> module for network adapter, routing table, and IP helper APIs.
/// </summary>
/// <remarks>
/// The set of exported <c>Iphlpapi.dll</c> functions can differ between Windows versions and installed Windows features.
...
/// ordinary application code; use this type when code needs direct access to Windows IP helper exports.
/// </remarks>
public static class Iphlpapi
{
    /// <summary>The canonical Windows module name for Iphlpapi.</summary>
    public const string LibraryName = "Iphlpapi.dll";

    private static readonly NativeModule s_module = new(LibraryName, typeof(Iphlpapi));
    private static readonly Lazy<IReadOnlyList<Export>> s_exports = new(GetExports, LazyThreadSafetyMode.ExecutionAndPublication);

    /// <summary>Gets the loaded native module handle for <c>Iphlpapi.dll</c>.</summary>
    /// <exception cref="DllNotFoundException">Thrown when Windows cannot load <c>Iphlpapi.dll</c>.</exception>
    public static nint ModuleHandle => s_module.ModuleHandle;

    /// <summary>Gets the full path to the loaded <c>Iphlpapi.dll</c> module.</summary>
    public static string ModulePath => s_module.ModulePath;

    /// <summary>Gets metadata for every exported <c>Iphlpapi.dll</c> function available on the current machine.</summary>
    public static IReadOnlyList<Export> Exports => s_exports.Value;

    /// <summary>Gets every named <c>Iphlpapi.dll</c> export available on the current machine.</summary>
    public static IReadOnlyList<string> ExportNames => s_module.ExportNames;

    /// <summary>Attempts to resolve a named <c>Iphlpapi.dll</c> export to its native function pointer.</summary>
    ...
    /// <summary>Resolves a named <c>Iphlpapi.dll</c> export to its native function pointer.</summary>
    /// <param name="name">The exact exported function name, including any ANSI/Unicode suffix such as <c>A</c> or <c>W</c>.</param>
    /// <returns>The native function pointer for the requested export.</returns>
    /// <exception cref="EntryPointNotFoundException">Thrown when the export does not exist in the loaded <c>Iphlpapi.dll</c>.</exception>
    public static nint GetExport(string name)
    {
        return s_module.GetExport(name);
    }

    /// <summary>Attempts to resolve a <c>Iphlpapi.dll</c> export by ordinal to its native function pointer.</summary>
    ...
    /// <summary>Resolves a <c>Iphlpapi.dll</c> export by ordinal to its native function pointer.</summary>
    /// <param name="ordinal">The 16-bit export ordinal.</param>
    /// <returns>The native function pointer for the requested export.</returns>
    /// <exception cref="EntryPointNotFoundException">Thrown when the ordinal does not exist in the loaded <c>Iphlpapi.dll</c>.</exception>
    public static nint GetExport(int ordinal)
    {
        return s_module.GetExport(ordinal);
    }

    /// <summary>Resolves a named <c>Iphlpapi.dll</c> export and converts it to a managed delegate.</summary>
    ...
    /// For example, bind <c>GetAdaptersAddresses</c> only to a delegate that matches the Windows SDK signature exactly.
    /// </remarks>
    ...
    /// <summary>Attempts to resolve a named <c>Iphlpapi.dll</c> export and convert it to a managed delegate.</summary>
    /// <typeparam name="TDelegate">The managed delegate type that exactly matches the native function signature.</typeparam>
    /// <param name="name">The exact exported function name, including any ANSI/Unicode suffix such as <c>A</c> or <c>W</c>.</param>
    ...
    /// <summary>Resolves a <c>Iphlpapi.dll</c> export by ordinal and converts it to a managed delegate.</summary>
    /// <typeparam name="TDelegate">The managed delegate type that exactly matches the native function signature.</typeparam>
    /// <param name="ordinal">The 16-bit export ordinal.</param>
    /// <returns>A delegate bound to the native function pointer.</returns>
    ...
    /// <summary>Attempts to resolve a <c>Iphlpapi.dll</c> export by ordinal and convert it to a managed delegate.</summary>
    /// <typeparam name="TDelegate">The managed delegate type that exactly matches the native function signature.</typeparam>
    /// <param name="ordinal">The 16-bit export ordinal.</param>
    ...
    /// <summary>Describes one native export from the loaded <c>Iphlpapi.dll</c> module.</summary>
    ...
    /// <param name="ForwardedTo">The forwarded export target, or <see langword="null" /> when the export is implemented in <c>Iphlpapi.dll</c>.</param>
