...
/// <summary>
/// Provides a version-tolerant wrapper over the native Windows <c>NCrypt.dll</c> module for Cryptography API: Next Generation key storage provider APIs.
/// </summary>
/// <remarks>
/// The set of exported <c>NCrypt.dll</c> functions can differ between Windows versions, installed Windows features, and configured key storage providers.
...
/// <c>TryGetFunction&lt;TDelegate&gt;</c>. Prefer managed cryptography abstractions for ordinary application code; use this
/// type when code needs direct access to CNG software, smart-card, TPM, hardware-backed, or provider-specific key storage exports.
/// </remarks>
public static class NCrypt
{
    /// <summary>The canonical Windows module name for NCrypt.</summary>
    public const string LibraryName = "NCrypt.dll";

    private static readonly NativeModule s_module = new(LibraryName, typeof(NCrypt));
    private static readonly Lazy<IReadOnlyList<Export>> s_exports = new(GetExports, LazyThreadSafetyMode.ExecutionAndPublication);

    /// <summary>Gets the loaded native module handle for <c>NCrypt.dll</c>.</summary>
    /// <exception cref="DllNotFoundException">Thrown when Windows cannot load <c>NCrypt.dll</c>.</exception>
    public static nint ModuleHandle => s_module.ModuleHandle;

    /// <summary>Gets the full path to the loaded <c>NCrypt.dll</c> module.</summary>
    public static string ModulePath => s_module.ModulePath;

    /// <summary>Gets metadata for every exported <c>NCrypt.dll</c> function available on the current machine.</summary>
    public static IReadOnlyList<Export> Exports => s_exports.Value;

    /// <summary>Gets every named <c>NCrypt.dll</c> export available on the current machine.</summary>
    public static IReadOnlyList<string> ExportNames => s_module.ExportNames;

    /// <summary>Attempts to resolve a named <c>NCrypt.dll</c> export to its native function pointer.</summary>
    ...
    /// <summary>Resolves a named <c>NCrypt.dll</c> export to its native function pointer.</summary>
    /// <param name="name">The exact exported function name.</param>
    /// <returns>The native function pointer for the requested export.</returns>
    /// <exception cref="EntryPointNotFoundException">Thrown when the export does not exist in the loaded <c>NCrypt.dll</c>.</exception>
    public static nint GetExport(string name)
    {
        return s_module.GetExport(name);
    }

    /// <summary>Attempts to resolve a <c>NCrypt.dll</c> export by ordinal to its native function pointer.</summary>
    ...
    /// <summary>Resolves a <c>NCrypt.dll</c> export by ordinal to its native function pointer.</summary>
    /// <param name="ordinal">The 16-bit export ordinal.</param>
    /// <returns>The native function pointer for the requested export.</returns>
    /// <exception cref="EntryPointNotFoundException">Thrown when the ordinal does not exist in the loaded <c>NCrypt.dll</c>.</exception>
    public static nint GetExport(int ordinal)
    {
        return s_module.GetExport(ordinal);
    }

    /// <summary>Resolves a named <c>NCrypt.dll</c> export and converts it to a managed delegate.</summary>
    ...
    /// <see cref="CallingConvention.Winapi" /> and the exact status-code, pointer, string, and handle types from the Windows SDK.
    /// For example, bind <c>NCryptOpenStorageProvider</c>, <c>NCryptOpenKey</c>, or <c>NCryptFreeObject</c> only to a matching delegate.
    /// </remarks>
    ...
    /// <summary>Attempts to resolve a named <c>NCrypt.dll</c> export and convert it to a managed delegate.</summary>
    /// <typeparam name="TDelegate">The managed delegate type that exactly matches the native function signature.</typeparam>
    /// <param name="name">The exact exported function name.</param>
    ...
    /// <summary>Resolves a <c>NCrypt.dll</c> export by ordinal and converts it to a managed delegate.</summary>
    /// <typeparam name="TDelegate">The managed delegate type that exactly matches the native function signature.</typeparam>
    /// <param name="ordinal">The 16-bit export ordinal.</param>
    /// <returns>A delegate bound to the native function pointer.</returns>
    ...
    /// <summary>Attempts to resolve a <c>NCrypt.dll</c> export by ordinal and convert it to a managed delegate.</summary>
    /// <typeparam name="TDelegate">The managed delegate type that exactly matches the native function signature.</typeparam>
    /// <param name="ordinal">The 16-bit export ordinal.</param>
    ...
    /// <summary>Describes one native export from the loaded <c>NCrypt.dll</c> module.</summary>
    ...
    /// <param name="ForwardedTo">The forwarded export target, or <see langword="null" /> when the export is implemented in <c>NCrypt.dll</c>.</param>
